...
DOCTEST_TEST_CASE("insert") {
	...
	for (isize n = 1; n < 32; ++n) {
		...
		for (isize i = 0; i < n + 1; ++i) {
			...
			LDLT_MAKE_STACK(
					stack, (Ldlt<T>::factor_req(n) | Ldlt<T>::insert_at_req(n + 1)));
			auto ldl = ldlt::Ldlt<T>{};
			ldl.factor(mat, LDLT_FWD(stack));
			ldl.insert_at(i, col, LDLT_FWD(stack));

			DOCTEST_CHECK(
					(mat_target - ldl.dbg_reconstructed_matrix()).norm() <= eps);
		}
	}
}
